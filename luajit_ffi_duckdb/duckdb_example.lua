...
local duckdb = ffi.load"duckdb"
local db = ffi.new("duckdb_database[1]")
local conn = ffi.new("duckdb_connection[1]")
local result = ffi.new("duckdb_result[1]")

print(duckdb.duckdb_open(":memory:", db))
print(duckdb.duckdb_connect(db[0], conn))
print(duckdb.duckdb_query(conn[0], [[SELECT 'Hello, World!';]], result))
local str = duckdb.duckdb_value_string(result, ffi.new("idx_t", 0), ffi.new("idx_t", 0))
ffi.C.printf("%s", str)
print(duckdb.duckdb_free(str))
print(duckdb.duckdb_destroy_result(result))
print(duckdb.duckdb_disconnect(conn))
print(duckdb.duckdb_close(db))
